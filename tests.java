...
public class tests {
    ...
    // testovani zda bylo datum dle kalendare
    static boolean date(int day, int month, int year) {
        if ((day >= 32 || day <= 0) && month == 1)
            ...
            return false;
        else if (year <= 1 || year >= 2003)
            return false;
        return true;
    }

    // metoda k odstraneni studenta z univerzity
    public static void deleteStudent(student student) {
        if (student instanceof technicanStudent) {
            technicanStudent.technicanStudents--;
            System.out.println("Student propusten z univerzity\n");
        } else if (student instanceof humanitaryStudent) {
            humanitaryStudent.humanitaryStudents--;
            System.out.println("Student propusten z univerzity\n");
        } else if (student instanceof combinadStudent) {
            combinadStudent.combinadStudents--;
            System.out.println("Student propusten z univerzity\n");
        }
    ...
    public static void zodiac(int month, int day) {
        if ((month == 12 && day >= 22) || (month == 1 && day <= 20)) {
            ...
            System.out.println("Vyskytla se chybička");
        }
    }
}